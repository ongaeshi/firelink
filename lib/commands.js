...
exports.textFromClipboard = function(label, linkdata) {
  if (customizeLinkPanel == null) {
    customizeLinkPanel = Panel.Panel({
      ...
      onShow: function() {
        this.postMessage({kind: "init", label: label, linkdata: linkdata});
      },
      onHide: function() {
      },
      onMessage: function(msg) {
        switch (msg.kind) {
         case 'ok':
          fl.copyLinkAndNotify(label, fl.currentLinkform(), msg.linkdata);
          this.hide();
          break;
         case 'close':
          this.hide();
